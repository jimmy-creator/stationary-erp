...
export function VendorPaymentReceipt() {
  ...
  // Admins can edit the underlying payment; mirrors isEditablePayment in MakePayment
  const canEdit = !isEmployee && po && payment.payment_method !== 'debit_note'
    ...
    <div className="max-w-3xl mx-auto print-area" style={{ '--invoice-accent': accent }}>
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 print-hide">
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          {canEdit && (
            <Link to={`/accounts-payable/${po.id}/pay?edit=${payment.id}`} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-zinc-800 border border-zinc-700 text-zinc-200 rounded-md hover:bg-zinc-700">
      ...
      <div className="print-only" style={{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '14px', border: '1px solid #cccccc' }}>
          <tbody>
            ...
            <tr>
              <td style={{ padding: '12px', backgroundImage: `linear-gradient(${accent}, ${accent})`, color: '#ffffff', borderTop: '2px solid #aaaaaa', fontSize: '12px', textTransform: 'uppercase', fontWeight: 700, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>Amount Paid</td>
