...
// Number to words for receipt footer (shared shape with SaleView)
...
export function PaymentReceipt() {
  ...
  const fetchData = async () => {
    try {
      const { data: pay, error } = await supabase.from('sale_payments').select('*').eq('id', id).single()
      ...
      if (pay.sale_id) {
        const { data: s } = await supabase
          .from('sales')
          .select('id, invoice_number, sale_date, customer_id, customer_name, grand_total, amount_paid')
          .eq('id', pay.sale_id).single()
        setSale(s)
        if (s?.customer_id) {
          const { data: c } = await supabase
            .from('customers').select('name, address, phone, email').eq('id', s.customer_id).single()
          setCustomer(c)
        }
      }
    } catch (err) {
  ...
  if (!payment) {
    return <div className="text-center py-8"><p className="text-zinc-500">Payment not found.</p><Link to="/accounts-receivable" className="text-teal-600 hover:underline">Back</Link></div>
  }

  const isAdjustment = payment.payment_method === 'discount' || payment.payment_method === 'credit_note'
  const docNumber = payment.receipt_number || (isAdjustment ? `Adjustment ${payment.id.substring(0, 8)}` : `Payment ${payment.id.substring(0, 8)}`)
  const totalSaleBalance = sale ? parseFloat(sale.grand_total || 0) - parseFloat(sale.amount_paid || 0) : 0
  const accent = store.invoice_header_color || DEFAULT_ACCENT
  // Admins can edit the underlying payment; mirrors isEditablePayment in CollectPayment
  const canEdit = !isEmployee && sale && payment.payment_method !== 'credit_note' && !(payment.payment_method === 'discount' && payment.reference)
  ...
  return (
    <div className="max-w-3xl mx-auto print-area" style={{ '--invoice-accent': accent }}>
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 print-hide">
        <div>
          {sale && <Link to={`/accounts-receivable/${sale.id}/collect`} className="text-teal-600 hover:underline text-sm mb-2 inline-block">&larr; Back to collection</Link>}
          <h1 className="text-xl lg:text-2xl font-bold text-white">{docNumber}</h1>
        </div>
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          {canEdit && (
            <Link to={`/accounts-receivable/${sale.id}/collect?edit=${payment.id}`} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-zinc-800 border border-zinc-700 text-zinc-200 rounded-md hover:bg-zinc-700">
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 print-hide">
        ...
        {sale && (
          <div className="border-t border-zinc-800 pt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-xs text-zinc-500 uppercase">Against Invoice</p>
              <Link to={`/sales/${sale.id}`} className="text-teal-400 hover:underline">{sale.invoice_number}</Link>
              <span className="text-xs text-zinc-500 ml-2">{fmtDate(sale.sale_date)}</span>
            </div>
            {payment.reference && (
              <div>
      ...
      <div className="print-only" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            {sale && <div style={{ fontSize: '12px', color: DARK }}>Against Invoice: <strong>{sale.invoice_number}</strong></div>}
          </div>
        </div>
        ...
        {sale && (
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '14px', border: '1px solid #dddddd' }}>
            <tbody>
              <tr>
                <td style={{ padding: '8px 10px', backgroundImage: 'linear-gradient(#f8f8f8, #f8f8f8)', borderRight: '1px solid #dddddd', fontSize: '10px', color: GRAY, textTransform: 'uppercase', fontWeight: 700 }}>Invoice Total</td>
                <td style={{ padding: '8px 10px', borderRight: '1px solid #dddddd', fontSize: '12px', color: DARK }}>{fmt(sale.grand_total)}</td>
                ...
                <td style={{ padding: '8px 10px', borderRight: '1px solid #dddddd', fontSize: '12px', color: '#0a7a3a' }}>{fmt(sale.amount_paid)}</td>
                ...
                <td style={{ padding: '8px 10px', fontSize: '12px', fontWeight: 700, color: totalSaleBalance > 0.01 ? '#cc0000' : DARK }}>{fmt(totalSaleBalance)}</td>
              </tr>
            </tbody>
          </table>
        )}
        ...
        <div style={{ marginTop: '40px', display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: LIGHT_GRAY }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ borderTop: '1px solid #999999', width: '180px', paddingTop: '4px', display: 'inline-block' }}>Customer Signature</div>
          </div>
        </div>
      </div>
    </div>
