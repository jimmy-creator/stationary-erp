...
export function CustomerPaymentReceipt() {
  ...
  const fetchData = async () => {
    try {
      const { data: pay, error } = await supabase.from('customer_payments').select('*').eq('id', id).single()
      ...
      if (pay.customer_id) {
        const { data: c } = await supabase
          .from('customers')
          .select('id, name, address, phone, email, opening_balance, opening_balance_date')
          .eq('id', pay.customer_id).single()
        setCustomer(c)
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto print-area" style={{ '--invoice-accent': accent }}>
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 print-hide">
        <div>
          {customer && <Link to={`/accounts-receivable/opening/${customer.id}/collect`} className="text-teal-600 hover:underline text-sm mb-2 inline-block">&larr; Back to collection</Link>}
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 print-hide">
        ...
        {customer && (
          <div className="border-t border-zinc-800 pt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-xs text-zinc-500 uppercase">Against</p>
              <p className="text-zinc-300">Opening Balance</p>
            </div>
            {payment.reference && (
              <div>
      ...
      <div className="print-only" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
          <div>
            ...
            <div style={{ fontSize: '16px', fontWeight: '700', color: DARK }}>{customer?.name || 'Customer'}</div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '12px', color: DARK }}>Against: <strong>Opening Balance</strong></div>
          </div>
        </div>
